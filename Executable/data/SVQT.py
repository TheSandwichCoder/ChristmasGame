    ...
    def getItem(self,pos):
        ...
        if position.y < functions.terrain[int(position.x)] * 100 + 300+500:
            ...
            if position.y < functions.terrain[int(position.x)] * 100 + 500 +500+ random.randint(-3,3):
                ...
                if (caveNoise[int((position.x)%(settings.NOISESIZE))][int((position.y)%(settings.NOISESIZE))]) == 0: #cave generation
                    ...
                    if (ironNoise[int(position.x % ironNoiseSize)][int(position.y % ironNoiseSize)]) == 0:  # cave generation
                        ...
                        if diamondNoise[int(position.x % ironNoiseSize)][int(position.y % ironNoiseSize)] == 0 or position.y < 2000+500:  # cave generation
                            self.type = 3
                        else:
                            self.type = 5
