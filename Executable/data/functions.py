...
size = int(settings.EldestNodeSize/settings.TILESIZE*16)

print("CHUNK BLOCKS:",settings.EldestNodeSize)
print("TILESIZE:",settings.TILESIZE)
print("WORLD SIZE:",settings.WorldSize)
# print(size)
# print(settings.WorldSize[0]*size)
print("CREATING GRASS TERRAIN")
terrain=[noise.valueAt(i/size*settings.WorldSize[0]) for i in range(size*settings.WorldSize[0])]
print("NOISE 1 CREATED")
#
# for i in range(100):
#     print(terrain[i])
...
resolutionOffset = 2**settings.resolution

caveNoiseArray = []

print("CREATING CAVE NOISE")
for i in range(settings.NUMOFNOISEMAPS):
    caveNoiseArray.append(Perlin.generateOreNoise(int(settings.NOISESIZE), int(settings.NOISESIZE), int(30*resolutionOffset), int(370*resolutionOffset)))
...
for i in range(settings.NUMOFNOISEMAPS*2):
    ...
    ironNoiseArray.append(Perlin.generateOreNoise(int(settings.NOISESIZE/4),int(settings.NOISESIZE/4),int(10*resolutionOffset),int(70*resolutionOffset)))
    print("NOISE",i,"CREATED")

diamondNoiseArray = []
print("CREATING DIAMOND ORE NOISE")
for i in range(settings.NUMOFNOISEMAPS*2):
    diamondNoiseArray.append(Perlin.generateOreNoise(int(settings.NOISESIZE/4),int(settings.NOISESIZE/4),int(10*resolutionOffset),int(10*resolutionOffset)))
